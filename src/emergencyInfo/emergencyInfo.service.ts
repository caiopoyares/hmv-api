...
export class EmergencyInfoService{
    constructor(
        @InjectRepository(EmergencyInfo) private emergencyInfoRepository: Repository<EmergencyInfo>,
    ...
    async findOne(id: string): Promise<EmergencyInfo> {
        ...
        if (!emergencyInfo)
            throw new NotFoundException(`An emergency order with id of ${id} hasn't been found.`);
      ...
      async add(createEmergencyInfoDto: CreateEmergencyInfoDto): Promise<EmergencyInfo> {
        const user = await this.userRepository.findOne(createEmergencyInfoDto.userId);

        if(!user){
          throw new NotFoundException(`A user with id of ${createEmergencyInfoDto.userId} hasn't been found.`);
        }
